...
namespace SpeedFit.Controllers
{
    public class DietaController : Controller
    {
        ...
        public IActionResult Visualizar(string id) // View visualização da dieta
        {
            Dieta = new Dieta();
            Dieta.Usuario = _context.Usuario.Where(u => u.Email == id).FirstOrDefault();
            var verificacao = _context.Dieta.Where(u => u.UsuarioId == Dieta.Usuario.Id).FirstOrDefault();
            if (verificacao != null)
            {
                Dieta = verificacao;
                Dieta.Alimentos = _context.Alimento.Where(u => u.DietaId == Dieta.Id).ToList();
